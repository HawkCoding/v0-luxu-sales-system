...
describe("applyTransition", () => {
  ...
  it("sets first travel date and repeat flag on first voucher sent", async () => {
    const now = new Date("2026-05-01T10:00:00.000Z")
    const { client, operations } = createFakeSupabase(
      {
        id: "booking-4",
  ...
  it("moves first travel date back and marks repeat client when an earlier second voucher is sent", async () => {
    const { client, operations } = createFakeSupabase(
      {
        id: "booking-5",
        stage: "voucher_sent",
  ...
  it("moves last travel date forward and marks repeat client when a later second voucher is sent", async () => {
    const { client, operations } = createFakeSupabase(
      {
        id: "booking-6",
        stage: "voucher_sent",
