...
- Default deposit is 25% — a booking cannot be confirmed without `deposit_paid = TRUE`
...
- The local app must run on **port 3000 only**: `http://localhost:3000`
- Do not start a second copy of the app on another port such as `3001`
- If port `3000` is already in use, assume the website/app is already running there and use that instance
- `pnpm dev` is intentionally pinned to port `3000`; if that port is occupied, stop and report that the existing app should be used
