-- =============================================================================
-- PRODUCTION BOOTSTRAP RESTORE
-- =============================================================================
-- Restores the minimum production bootstrap data after a full business-data wipe:
-- countries, country aliases, base locations, default app settings, storage
-- buckets/policies, and staff profiles for already-created Auth users.
--
-- This script is idempotent. It intentionally does NOT insert directly into
-- auth.users. Create missing users through Supabase Auth first, or run:
--   pnpm production:restore:users
-- =============================================================================
...
SELECT 'countries' AS check_name, count(*) AS count FROM public.countries
UNION ALL SELECT 'country_aliases', count(*) FROM public.country_aliases
UNION ALL SELECT 'locations', count(*) FROM public.locations
UNION ALL SELECT 'app_settings_required_keys', count(*) FROM public.app_settings WHERE key IN ('business_name', 'default_deposit_percentage', 'session_timeout_minutes')
UNION ALL SELECT 'active_admin_profiles', count(*) FROM public.profiles WHERE clearance_level = 'admin' AND is_active = true;
