...
export const MANAGER = {
  ...
  // Seeded user_id for Dirk Rossouw (supabase/seed.sql, clearance_level=manager).
  userId: "00000000-0000-0000-0000-0000000000a3",
} as const

// Other seeded staff used as reassignment targets / "owned by another consultant".
export const CONSULTANT_LEONIE = "00000000-0000-0000-0000-0000000000a2"
export const CONSULTANT_MONADE = "00000000-0000-0000-0000-0000000000a4"

export const MANAGER_STORAGE_STATE = resolve(process.cwd(), "tests/qa/.auth/manager.json")
const SCREENSHOT_DIR = resolve(process.cwd(), "tests/qa/screenshots")
const REPORT_PATH = resolve(process.cwd(), "tests/qa/reports/manager-qa.md")
...
// the markdown deliverable at tests/qa/reports/manager-qa.md. Structure mirrors
// the Consultant report (Prompt 1) so the two read identically.
// ---------------------------------------------------------------------------
class ManagerReport {
  ...
  async shot(page: Page, num: number, label: string): Promise<string> {
    ...
    const file = `m${num}-${label.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}.png`
  ...
  write(): string {
    ...
    const L: string[] = []
    L.push("# Manager Role — End-to-End QA Findings (Phase 33 UAT)")
    L.push("")
    ...
    L.push(`- **Acting role:** Manager — ${MANAGER.email} (clearance_level=manager)`)
    ...
    L.push("## Coverage map — todo.md Phase 33 Manager UAT")
    L.push("")
    L.push("| todo.md bullet | Criterion | Demonstrably passing? |")
    L.push("|----------------|-----------|------------------------|")
    const map: [string, number][] = [
      ["Can edit + add/remove templates", 1],
      ["Can manage suppliers", 2],
      ["Can manage rates (overlap prevention)", 3],
      ["Can view reports + export CSV", 4],
      ["Can reassign jobs", 5],
      ["Can resolve errors", 6],
    ]
...
export const report = new ManagerReport()
