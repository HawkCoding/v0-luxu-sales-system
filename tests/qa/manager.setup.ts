...
import { MANAGER, MANAGER_STORAGE_STATE } from "./manager.fixtures"

// Logs in as the seeded `manager` user (dirk) and saves the session so the
// spec runs as a Manager rather than the admin/consultant used elsewhere.
async function globalSetup(): Promise<void> {
  configurePlaywrightRuntime()
  loadQaEnv()

  // Sanity-check the seeded manager exists; fail loudly with guidance if not.
  ...
  if (!data || data.clearance_level !== "manager") {
    throw new Error(
      `Seeded manager ${MANAGER.email} (clearance_level=manager) not found. ` +
        `Run \`pnpm db:reset\` to load the seed before running the Manager QA suite.`,
    )
  }

  mkdirSync(dirname(MANAGER_STORAGE_STATE), { recursive: true })
  ...
  try {
    ...
    if (!loggedIn) {
      throw new Error(`Failed to log in as manager ${MANAGER.email}. Current URL: ${page.url()}`)
    }
