...
export async function POST(req: Request) {
  ...
  const { data: supplier, error: supplierError } = await adminSupabase
    ...
    .select("id, slug, kind, pricing_mode, transfer_pricing_basis, accommodation_pricing_basis, status, name, email, phone, website, location, location_id, location_area_id, description, notes, active, single_supplement_pct, infant_max_age, child_max_age, default_time_start, default_time_end, inclusions, exclusions, street_address, emergency_phone, default_contact_name, parent_supplier_id, long_journey_min_days, train_only_note, quote_suite_detail, check_in_offset_minutes, suite_phrase_pattern, sells_standalone, email_match_phrases, created_at, updated_at")
    .single()
