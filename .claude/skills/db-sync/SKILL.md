...
- Schema drift on a target that is in sync on migrations almost always means a manual edit in the Supabase dashboard. On a BEHIND target the unapplied migrations are the drift — push them and re-check.
- Read the generated `tmp-db-sync/status-*/<target>/remote-to-local.sql` and name the objects that differ — never just relay the statement count.
