-- replace_booking_transport_requests wrote package_leg_id verbatim from the client payload, which
-- was safe while every leg (catalogue or Build Booking) was a real package_legs row. Now that
-- Build Booking legs are booking_services rows instead, passing one of those ids through
-- package_leg_id would violate its foreign key. Add service_id alongside it so a request can be
-- tied to either kind of leg -- never both, same convention as build-from-package.ts's transport
-- matching and the voucher block builder's leg-reference reshaping.
