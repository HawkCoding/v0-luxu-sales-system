-- Add external-facing description to suppliers (used on vouchers and quotations)
ALTER TABLE public.suppliers ADD COLUMN IF NOT EXISTS description text;

-- Store supplier description alongside quote line items for email rendering
ALTER TABLE public.quote_line_items ADD COLUMN IF NOT EXISTS supplier_description text;

-- Update replace_quote_line_items RPC to carry supplier_description through
